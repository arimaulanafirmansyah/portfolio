...
router.get('/game/pb', async (req, res, next) => {
  ...
  if(listkey.includes(Apikey)){
     ...
     request.post(`https://api.duniagames.co.id/api/transaction/v1/top-up/inquiry/store?productId=17&itemId=82&catalogId=138&paymentId=822&gameId=${user}&product_ref=REG`, function (error, response, body) {
         try {
          var data = JSON.parse(body)
             res.json({
                status : true,
